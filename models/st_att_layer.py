...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, ft_size, time_len=20, domain="spatial"):
...
if __name__ == "__main__":
    # batch size * time len * node number * channel number = 32 * 20 * 22 * 128
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    x = torch.randn(32, 20, 22, 128).to(device)
    s_att = ST_ATT_Layer(input_size=128,output_size= 128, h_num=32, h_dim=8, dp_rate=0.4, domain="spatial", time_len = 20)
    s_att.to(device)
    out = s_att(x)
    # batch size * (time len * node number) * channel number = 32 * 440 * 128
    print(out.shape)