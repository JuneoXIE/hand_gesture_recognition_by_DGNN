...
class DGNN_ATT(nn.Module):
    def __init__(self, num_class=14, num_point=22, num_person=1, graph_args=dict(), in_channels=3):
        ...
        #self.s_att = ST_ATT_Layer(input_size=128,output_size= 128, h_num=24, h_dim=8, dp_rate=0.2, domain="spatial", time_len = 20)
        self.t_att = ST_ATT_Layer(input_size=128,output_size= 128, h_num=24, h_dim=8, dp_rate=0.2, domain="temporal", time_len = 20)
    ...
    def forward(self, fv, fe):
        ...
        fv, fe = self.l2(fv, fe)
        #fv, fe = self.l3(fv, fe)
        # Shape: (N,C,T,V), C is same for fv/fe,  32 * 64 * 20 * 22

        # Concat node and edge features
        x = torch.cat((fv, fe), dim=1) # 32 * 128 * 20 * 22
        x = x.permute(0, 2, 3, 1).contiguous() # 32 * 20 * 22 * 128
...
if __name__ == "__main__":
    import sys
    sys.path.append('..')
    model = DGNN(graph='graph.directed_ntu_rgb_d.Graph')
